...
export async function createCampaign({ actor, input, ip, userAgent }) {
  if (!canPublishPosts(actor)) throw forbidden('Only an Editor or Admin can create a campaign.');
  const row = (
    await query(
...
export async function updateCampaign({ id, actor, input, ip, userAgent }) {
  if (!canPublishPosts(actor)) throw forbidden('Only an Editor or Admin can change a campaign.');
...
export async function deleteCampaign({ id, actor, ip, userAgent }) {
  if (!canPublishPosts(actor)) throw forbidden('Only an Editor or Admin can delete a campaign.');
  const existing = await getRow(id);
  if (!existing) throw notFound('Campaign not found');
